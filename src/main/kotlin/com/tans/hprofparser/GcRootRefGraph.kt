...
sealed class RefField {
    ...
    data class MemberField(val field: com.tans.hprofparser.MemberField) : RefField()

    data class StaticField(val field: com.tans.hprofparser.StaticField) : RefField()
            ...
            is ValueHolder.BooleanHolder -> {
                visitInstanceChildren.add(
                    RefTreeNode.Primitive(
                        dominatorField = if (isMemberField) RefField.MemberField(memberField!!.field) else RefField.StaticField(staticField!!),
                        value = value.value
                    )
                )
            }
                ...
                if (i == null) {
                    ...
                    when (i) {
                        ...
                        is Instance.ByteArrayInstance -> {
                            visitInstanceChildren.add(RefTreeNode.PrimitiveArray(
                                dominatorField = if (isMemberField) RefField.MemberField(memberField!!.field) else RefField.StaticField(staticField!!),
                                array = i.array
                            ))
                        }
                            ...
                            for ((index, e) in i.elements.withIndex()) {
                                val newNode = RefTreeNode.ObjectArrayElement(
                                    dominatorField = RefField.ArrayElementField(
                                        field = if (isMemberField) RefField.MemberField(memberField!!.field) else RefField.StaticField(staticField!!),
                                        index = index
                                    ),
                                    value = if (e != null) {
                                        RefTree(
